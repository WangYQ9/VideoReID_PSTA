...
class gap_block(nn.Module):
    ...
    def forward(self, feat_map):
        ...
        gap_feat_map = torch.stack(gap_feat_map0, 1)
        ...
        gap_feat_vect = self.avg(gap_feat_map.view(b*seq_len, -1, w, h)).view(b, seq_len, -1)
        ...
        for i in range(seq_len):
            idx = 2 * i
            para_0 = self.sigmoid(self.para_fc1_0(torch.cat([gap_feat_vect[:, i, :], feat_vect[:, idx + 1, :]], 1)))
            para_1 = self.sigmoid(self.para_fc1_0(torch.cat([gap_feat_vect[:, i, :], feat_vect[:, idx, :]], 1)))
            para_0 = self.relu(self.para_fc1_1(para_0)).view(b, -1, 1, 1)
            para_1 = self.relu(self.para_fc1_1(para_1)).view(b, -1, 1, 1)
            gap_map = gap_feat_map[:, i, :, :, :] + para_0 * feat_map[:, idx, :, :, :] + para_1 * feat_map[:, idx + 1, :, :, :]
            re_gap_feat_map.append(gap_map)

        re_gap_feat_map = torch.stack(re_gap_feat_map, 1)
        feature = self.avg_3d(re_gap_feat_map.permute(0, 2, 1, 3, 4)).view(b, -1)

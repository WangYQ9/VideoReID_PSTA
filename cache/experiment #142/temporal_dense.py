...
class tem_dense(nn.Module):
    ...
    def __init__(self, num_classes, model_name, pretrain_choice, seq_len, dropout=0):
        ...
        t = seq_len
        self.layer1 = gap_block(inplanes=self.in_planes, mid_planes=int(self.in_planes * 0.5), seq_len = t / 2)
        t = t / 2
        self.layer2 = gap_block(inplanes=self.in_planes, mid_planes=int(self.in_planes * 0.5), seq_len = t / 2)
        t = t / 2
        self.layer3 = gap_block(inplanes=self.in_planes, mid_planes=int(self.in_planes * 0.5), seq_len = t / 2)
    ...
    def forward(self, x, pids=None, camids=None):
        ...
        feature_ord3 = self.down_channel(feature_ord3)

        if self.dropout > 0:
            feature_ord3 = self.drop(feature_ord3)

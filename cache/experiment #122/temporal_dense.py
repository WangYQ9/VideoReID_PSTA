...
class gap_block(nn.Module):
    ...
    def forward(self, feat_map):
        ...
        for idx in range(0, t, 2):
            gap_map0 = para[:, idx + 1, :, :, :] * feat_map[:, idx, :, :, :] + para[:, idx, :, :, :] * feat_map[:, idx + 1, :, :, :]
            gap_map0 = gap_map0 ** 2
        ...
        # center_feat_vect = torch.mean(gap_feat_vect, 1).view(b, 1, -1)
        # weight_score = torch.cosine_similarity(gap_feat_vect, center_feat_vect, dim=2)
        # weight_score = self.softmax(weight_score).view(b, -1, 1)
        # re_gap_feat_vect = gap_feat_vect * weight_score
        # feature = torch.sum(re_gap_feat_vect, 1)
        feature = torch.mean(gap_feat_vect, 1).view(b, -1)
        gap_feat_map = gap_feat_map.view(b, seq_len, -1, w, h)
...
class tem_dense(nn.Module):
    ...
    def forward(self, x, pids=None, camids=None):
        ...
        # center_feat_vect_0 = torch.mean(feat_vect_0, 1).view(b, 1, -1)
        # weight_score = torch.cosine_similarity(feat_vect_0, center_feat_vect_0, dim=2)
        # weight_score = self.softmax(weight_score).view(b, -1, 1)
        # re_feat_vect_0 = feat_vect_0 * weight_score
        # feature_0 = torch.sum(re_feat_vect_0, 1)
        feature_0 = torch.mean(feat_vect_0, 1).view(b, -1)
        ...
        feature_list.append(feature_ord1)
        ...
        if self.training:
            cls_score_list.append(self.classifier[0](BN_feature_ord0))

...
class gap_block(nn.Module):
    ...
    def forward(self, feat_map):
        ...
        for i in range(seq_len):
            idx = 2 * i
            para_0 = self.sigmoid(self.para_fc1_0(torch.cat([gap_feat_vect0[:, i, :], feat_vect[:, idx, :]], 1)))
            para_1 = self.sigmoid(self.para_fc1_0(torch.cat([gap_feat_vect0[:, i, :], feat_vect[:, idx + 1, :]], 1)))

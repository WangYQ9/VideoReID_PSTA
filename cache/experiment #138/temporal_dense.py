...
class gap_block(nn.Module):
    ...
    def __init__(self, inplanes, mid_planes, seq_len):
        ...
        self.para_fc1_0 = nn.Linear(in_features=inplanes, out_features=int(inplanes / 16))
        self.para_fc1_1 = nn.Linear(in_features=int(inplanes / 16), out_features=inplanes)
    ...
    def forward(self, feat_map):
        ...
        for i in range(seq_len):
            idx = 2 * i
            para_0 = self.sigmoid(self.para_fc1_0(feat_para[:, idx, :]))
            para_1 = self.sigmoid(self.para_fc1_0(feat_para[:, idx + 1, :]))

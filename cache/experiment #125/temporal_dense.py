...
class gap_block(nn.Module):
    ...
    def __init__(self, inplanes, mid_planes, seq_len):
        ...
        self.para_fc0_1 = nn.Linear(in_features=int(inplanes / 8), out_features=inplanes)
    ...
    def forward(self, feat_map):
        ...
        para = para.view(b, t, -1)

        add_para = []
        for i in range(0, t, 2):
            add_para.append(torch.cat([para[:, i, :], para[:, i+1, :]], 1))
            add_para.append(torch.cat([para[:, i, :], para[:, i+1, :]], 1))
        add_para = torch.stack(add_para, 1).view(b*t, -1)

        para = self.relu(self.para_fc0_1(add_para))
        para = para.view(b, t, -1, 1, 1)
        gap_feat_map0 = []

        for idx in range(0, t, 2):
            gap_map0 = para[:, idx, :, :, :] * feat_map[:, idx, :, :, :] + para[:, idx, :, :, :] * feat_map[:, idx, :, :, :]
...
class tem_dense(nn.Module):
    ...
    def forward(self, x, pids=None, camids=None):
        ...
        feature_0 = self.cat_vect(feat_vect_0)
        feature_0 = self.relu(feature_0).view(b, -1)
        ...
        feature_list = []
        feature_ord0 = feature_0
        feature_list.append(feature_ord0)

        cat_feat_1 = torch.stack([feature_0, feature_1], 1)
        cat_feat_2 = torch.stack([feature_0, feature_1, feature_2], 1)
        cat_feat_3 = torch.stack([feature_0, feature_1, feature_2, feature_3], 1)

        feature_ord1 = self.cat_conv[0](cat_feat_1).view(b, -1)
        feature_ord2 = self.cat_conv[1](cat_feat_2).view(b, -1)
        feature_ord3 = self.cat_conv[2](cat_feat_3).view(b, -1)

        feature_ord1 = self.relu(feature_ord1)
        feature_ord2 = self.relu(feature_ord2)
        feature_ord3 = self.relu(feature_ord3)

        BN_feature_ord0 = self.bottleneck(feature_ord0)
        ...
        feature_list.append(BN_feature_ord0)
        feature_list.append(BN_feature_ord1)
        feature_list.append(BN_feature_ord2)
        feature_list.append(BN_feature_ord3)

        cls_score_list = []
        torch.cuda.empty_cache()

        if self.training:
            cls_score_list.append(self.classifier[0](feature_ord0))
            cls_score_list.append(self.classifier[1](feature_ord1))
            cls_score_list.append(self.classifier[2](feature_ord2))
            cls_score_list.append(self.classifier[3](feature_ord3))

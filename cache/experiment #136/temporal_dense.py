...
class gap_block(nn.Module):
    ...
    def __init__(self, inplanes, mid_planes, seq_len):
        ...
        self.para_fc0_0 = nn.Linear(in_features=int(2 * inplanes), out_features=int(inplanes / 16))
        self.para_fc0_1 = nn.Linear(in_features=int(inplanes / 16), out_features=inplanes)
        self.para_fc0_2 = nn.Linear(in_features=int(inplanes / 16), out_features=inplanes)
        # self.para_fc0_3 = nn.Linear(in_features=int(inplanes / 8), out_features=inplanes)
        ...
        if seq_len != 1:
            ...
            self.cat_vect.apply(weights_init_kaiming)  # problem ay be make

        self.para_fc0_0.apply(weights_init_kaiming)
        self.para_fc0_1.apply(weights_init_kaiming)
        self.para_fc0_2.apply(weights_init_kaiming)
        # self.para_fc0_3.apply(weights_init_kaiming)
...
class tem_dense(nn.Module):
    ...
    def __init__(self, num_classes, model_name, pretrain_choice, seq_len):
        ...
        self.classifier = nn.ModuleList([nn.Linear(self.plances, self.num_classes, bias=False) for i in range(3)])
    ...
    def forward(self, x, pids=None, camids=None):
        ...
        if self.training:
            cls_score_list.append(self.classifier[0](BN_feature_ord1))
            cls_score_list.append(self.classifier[1](BN_feature_ord2))
            cls_score_list.append(self.classifier[2](BN_feature_ord3))
